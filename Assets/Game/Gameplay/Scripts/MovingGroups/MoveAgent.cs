...
public sealed class MoveAgent : MonoBehaviour
{
    ...
    {
        this.unit.SetData(new MoveStepData
        {
            direction = direction,
            completed = false
        });
    ...
    private void StartAvoidObstacle(Vector3 avoidDirection)
    {
        if (this.avoidObstacleCoroutine == null)
        {
            this.avoidObstacleCoroutine = this.StartCoroutine(this.AvoidObstacleRoute(avoidDirection));
        }
    }
    ...
    private void StopAvoidObstacle()
    {
        if (this.avoidObstacleCoroutine != null)
        {
            ...
            if (this.unit.HasData<MoveStepData>())
            {
    ...
    private IEnumerator AvoidObstacleRoute(Vector3 moveDirection)
    {
        while (true)
        {
